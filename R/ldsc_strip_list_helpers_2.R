...
make_trait_sets <- function(
  ...
  # R needs 1.25 GB ish just to exist
  R_intercept <- 1.73
  # leave room for temp objects (this is an overestimate)
  overhead <- 0.1*ntraits 
  # should scale w/ # inputs?

  mem_for_traits_gb <- memory_limit_gb - (R_intercept + overhead)
  # convert to bytes because we think in R a float (total N_numbers) = 8 bytes to store in matrix
  mem_for_traits <- mem_for_traits_gb * 1e9

  print(paste('mem avail for traits in bytes:',mem_for_traits))
  ...
  max_traits_per_block <- floor(mem_for_traits / (32 * L))
  print(paste('max traits per block from mem avail for traits:',max_traits_per_block))
